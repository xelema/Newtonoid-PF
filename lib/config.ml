...
(* Tests *)
let%test "default_bounds valid" =
  default_bounds.x_min < default_bounds.x_max &&
  default_bounds.y_min < default_bounds.y_max

let%test "default_physics positive gravity" =
  default_physics.gravity > 0.

let%test "default_physics positive ball_radius" =
  default_physics.ball_radius > 0.

let%test "default_paddle positive dimensions" =
  default_paddle.width > 0. && default_paddle.height > 0.

let%test "default_lives positive" =
  default.initial_lives > 0

let%test "make with defaults" =
  let cfg = make () in
  cfg.initial_lives = 3 && cfg.dt = default.dt

let%test "make with custom lives" =
